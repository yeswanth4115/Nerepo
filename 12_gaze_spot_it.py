...
def is_point_inside_target(px, py, target):
    tolerance = TARGET_ZONE_TOLERANCE
    return (
        target.x1 - tolerance <= px <= target.x2 + tolerance
        and target.y1 - tolerance <= py <= target.y2 + tolerance
    )
...
def draw_scene(canvas, target_zones, selected_index, gaze_x=None, gaze_y=None):
    ...
    if gaze_x is not None and gaze_y is not None and SHOW_GAZE_CURSOR:
        cv2.circle(canvas, (int(gaze_x), int(gaze_y)), 10, (0, 255, 255), -1)
        cv2.circle(canvas, (int(gaze_x), int(gaze_y)), 18, (0, 255, 255), 2)
    ...
    with open(csv_path, "w", newline="") as csv_file:
        ...
        for trial_id in range(1, NUM_TRIALS + 1):
            ...
            while True:
                ...
                in_selected = is_point_inside_target(gaze_x, gaze_y, selected_target)
                other_target_hits = [
                    idx for idx, zone in enumerate(target_zone_map) if idx != target_index and is_point_inside_target(gaze_x, gaze_y, zone)
                ]
                ...
                if in_selected:
                    ...
                    inside_target_zone_for_ms += 33.0
                else:
                    fixation_armed = False
                    inside_target_zone_for_ms = 0.0

                if other_target_hits:
                    distractor_entry_count += len(other_target_hits)
                ...
                if successful_acquisition:
                    ...
                    cv2.putText(
                        canvas,
                        f"Fixation: {int(inside_target_zone_for_ms)} ms / {MIN_FIXATION_MS} ms",
                        (40, 150),
                        cv2.FONT_HERSHEY_SIMPLEX,
                        0.8,
                        (255, 255, 255),
...
if __name__ == "__main__":
    ...
    print("Instructions: look at the smiling face and maintain fixation for the required duration.")
